package org.progingo.infrastructure.auth;
...
public class JWTRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
        ...
        UserBO userCache = tokenUtil.getUserCache(token);//将token传递给工具类，从缓存中查找是否有对应token的用户信息(因为正常用户登录后信息都保存在redis中了，现在就是逆向操作，去redis中查找，如果查找不到自然就说明token是非法或者过期的，查找到了则就是身份校验成功)
